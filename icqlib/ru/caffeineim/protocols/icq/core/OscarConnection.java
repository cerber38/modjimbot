...
public class OscarConnection {
    ...
    public OscarConnection(String host, int port, String userId, String password, boolean webAware, boolean birthday) {
        this(host, port, userId, password, new Tweaker(webAware, birthday), null);
    }
    ...
    public OscarConnection(String host, int port, String userId, String password,
        ...
        client = new OscarClient(host, port, analyser, factory, this);
    ...
    public synchronized void sendFlap(Flap flapPacket) {
        if (flapPacket.getSequenceNumber() == Integer.MAX_VALUE) { // not assigned yet..
            ...
            if (flapSeqNrs > 0x7fff)   //by Пушкожук 
                flapSeqNrs = 0;
    ...
    public void pingNeeded() {  //by Пушкожук
        Flap ping=new Flap(5);
        sendFlap(ping);
    }
